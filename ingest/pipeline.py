...
class Pipeline:
    ...
    def _human(self, obs: Observation) -> list[dict[str, Any]]:
        ...
        source = self.db.source(obs.source_id) or {"alpha": 1.0, "beta": 1.0}
        previous = [
            ClaimLike(c["id"], c["claim_text"] or "", c["source_id"] or "", c.get("channel", "unknown"), bool(c["is_firsthand"]))
            for c in self.db.claims() if c["settlement_id"] == obs.settlement_id
        ]
        current = ClaimLike(obs.obs_id, obs.text_en or "", obs.source_id, obs.channel, obs.is_firsthand)
        clusters = collapse([*previous, current])
        cluster = next(group for group in clusters if any(c.claim_id == obs.obs_id for c in group))
        root = cluster[0]
        independent = independent_source_count(cluster)
        claim = self._claim_payload(obs, root.claim_id, len(cluster), independent)
        self.db.save_claim(claim)
        if root.claim_id != obs.obs_id:
            return []
        reliability = source["alpha"] / (source["alpha"] + source["beta"])
        lr = human_lr(obs.severity_hint, reliability, independent, obs.is_firsthand)
        mode = HAZARD_MODE.get(obs.hazard, FailureMode.CASUALTY if any(s in obs.subjects for s in ("injured", "dead", "people_trapped")) else FailureMode.INUNDATION)
        return [self._evidence(obs, mode, lr, "human_report")]
    ...
    @staticmethod
    def _evidence(obs: Observation, mode: FailureMode, lr: float, group: str) -> dict[str, Any]:
        return {"settlement_id": obs.settlement_id, "channel": obs.channel, "failure_mode": mode.value, "log_lr": math.log(max(0.001, lr)), "correlation_group": group, "ts": obs.ts.isoformat(), "raw_ref": obs.obs_id}
